...
namespace LoyaltyCandy.NNSGovernance.Models
{
	public class ProposalInfo
	{
		...
		public ProposalInfo(OptionalValue<ProposalId> id, int status, int topic, OptionalValue<GovernanceError> failureReason, Dictionary<ulong, Ballot> ballots, ulong proposalTimestampSeconds, ulong rewardEventRound, OptionalValue<ulong> deadlineTimestampSeconds, ulong failedTimestampSeconds, ulong rejectCostE8s, OptionalValue<DerivedProposalInformation> derivedProposalInformation, OptionalValue<Tally> latestTally, int rewardStatus, ulong decidedTimestampSeconds, OptionalValue<Proposal> proposal, OptionalValue<NeuronId> proposer, ulong executedTimestampSeconds, OptionalValue<ulong> totalPotentialVotingPower)
		{
			this.Id = id;
			this.Status = status;
			this.Topic = topic;
